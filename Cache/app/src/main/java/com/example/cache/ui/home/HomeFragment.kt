...
class HomeFragment : DaggerFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val totalInvested: TextView = binding.root.findViewById(R.id.total_invested)
        val totalSavings: TextView = binding.root.findViewById(R.id.total_savings)

        homeViewModel.investmentAccounts.observe(viewLifecycleOwner, Observer { investmentAccounts ->
            val balances = investmentAccounts.map{ it.balance }.fold(Money()) { acc, e -> acc + e }
            val savings = investmentAccounts.map{ it.liquidBalance }.fold(Money()) { acc, e -> acc + e }
            val invested = balances - savings

            totalInvested.text = invested.toString()
            totalSavings.text = savings.toString()
        })

        val viewPortfolioButton: ConstraintLayout = binding.root.findViewById(R.id.view_portfolios_button)
